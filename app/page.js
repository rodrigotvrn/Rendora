...
var C = {
    navy: "#192B47", navyLight: "#1E3250", navyMid: "#243A5C", surface: "#162842", border: "#2A4266",
    accent: "#00BDE4", green: "#2EAD4B", gold: "#E8A832", danger: "#E5484D", warning: "#F5A623",
    white: "#F1F5F9", text: "#CBD5E1", textMuted: "#8B9FC0", textDim: "#5A7194",
};
...
{[{ label: "RECEITAS", valor: "0", icon: "📋", cor: C.accent }, { label: "INGREDIENTES", valor: "0", icon: "🥕", cor: C.green }, { label: "CLIENTES ATIVOS", valor: "0", icon: "🏢", cor: C.gold }, { label: "CARDAPIOS/MES", valor: "0", icon: "📅", cor: C.warning }, { label: "CUSTO MEDIO", valor: "R$ 0,00", icon: "💰", cor: C.green }, { label: "COMENSAIS/DIA", valor: "0", icon: "👥", cor: C.accent }].map(function (k) {
...
function ModClientes() {
    return (
        ...
        <div><div style={{ fontSize: 20, fontWeight: 800, color: C.white }}>Clientes</div><div style={{ fontSize: 12, color: C.textMuted }}>0 clientes</div></div>
...
function ModCompras() {
    return (
      ...
      <Vazio icon="🛒" titulo="Nenhum pedido de compra" msg="Os pedidos de compra aparecerão aqui." />
  </div>
    );
}
...
function ModPreparacoes() {
    return (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div><div style={{ fontSize: 20, fontWeight: 800, color: C.white }}>Preparacoes / Receitas</div></div>
          <button style={sBtnGreen}>+ Nova Receita</button>
  </div>
...
export default function RendoraApp() {
    ...
    var _p = useState("dash");
    var page = _p[0];
    var setPage = _p[1];
    var _c = useState(false);
    var collapsed = _c[0];
    var setCollapsed = _c[1];
    var _u = useState(null);
    var user = _u[0];
    var setUser = _u[1];
    var _pf = useState(null);
    var perfil = _pf[0];
    var setPerfil = _pf[1];
  ...
  function handleLogout() {
        supabase.auth.signOut().then(function () { router.push("/login"); });
  }
  ...
  else if (page === "prep") content = <ModPreparacoes />;
...
{NAV.map(function (item) {
              ...
              return (<div key={item.id} onClick={function () { setPage(item.id); }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 10, cursor: "pointer", marginBottom: 2, background: active ? C.accent + "15" : "transparent", borderLeft: active ? "3px solid " + C.accent : "3px solid transparent" }}>
